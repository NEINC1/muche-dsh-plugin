...
export class BackendWs {
  ...
  #wsUrl() {
    ...
    // 通道由构造参数决定（dsh-bridge：反向桥接，后端独立桥接池；空串＝面板池，
    // 面板下行复用（SSE 替代通道），后端按无 channel 判面板）。path 前缀原样保留：
    // 远端 `http(s)://<公网>/api` 拼出 `/api/ws`，经 nginx 反代到达后端。
    const query = `token=${encodeURIComponent(this.apiKey)}` + (this.channel ? `&channel=${encodeURIComponent(this.channel)}` : '')
