 ...
 *
 * 小沐接入的正式形态（2026-08-14 决策，见 muche 仓库 docs 与 PROGRESS）：
 * 小沐聊天 = 插件自绘浮层面板 + 直连后端 HTTP，**不进 dsh agent 循环、不建
 * dsh 会话**。原因（实测结论，勿改回）：
 *  - dsh 会话的隐藏机制与可聊天互斥：归档会在当前会话被归档时被客户端
 *    强制清空选择；header.origin='subagent' 会触发 api-remotes 的
 *    subagent 路由门控（普通 session.prompt 被 agent-busy 拒绝）；
 *  - 会话内 LLM 调用会把 AGENTS.md(agent-instructions)/runtime context
 *    (plugin)/委派通知等注入成 role='user' 的消息，需要适配器过滤，且
 *    显示层仍有残留——面板形态从根上消除注入面。
 *
 * Host 职责：注册 settings 命名空间（后端地址+API key，每用户本机配置）、
 * 注册 /api/muche/* HTTP 路由（聊天/历史/配置/连接测试，浏览器同源直调）+
 * 反向桥接（dsh-bridge.js：小沐经用户本机插件调用 dsh 会话，出站常驻 WS）。
 * 客户端代码见 client/client.js（导出 inject/apply）。
 */
import { registerConfigNamespace, importLegacyConfig } from './config.js'
...
export function apply(ctx) {
  registerConfigNamespace(ctx)
  void importLegacyConfig(ctx)
  registerRoutes(ctx)
  registerWsProxy(ctx)
