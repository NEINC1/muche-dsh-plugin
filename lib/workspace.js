...
/** 显式解析工作区目录路径（settings 配置优先；空 = os.homedir 推导，跨平台）。 */
export function resolveWorkspacePath(ctx) {
...
export async function ensureWorkspace(ctx) {
  const wsPath = resolveWorkspacePath(ctx)
  ...
  // session.create 静默落默认 cwd（实测 2026-08-15：会话建在 /home/ubuntu 未挂工作区）
...
export async function listWorkspaceSessions(ctx, workspaceId, limit = 5) {
  ...
  for (const sessionId of ws.sessionIds) {
    ...
    try {
      ...
      // （不是 SessionTitleObservation——那要 readTitleSnapshot；实测取错层
      //  会拿到 undefined.title 变空串，2026-08-15 实连暴露）
