 ...
 * （settings.workspacePath 可配置，Windows 兼容零路径硬编码），与用户代码
 ...
 */
async function promptSession(ctx, sessionId, text, signal) {
  const sc = ctx.get('sessionController')
  if (!sc) throw new Error('缺少 sessionController，反向桥接不可用')
  await sc.prompt({
 ...
 */
export async function executeDshTask(ctx, { task, session_id, agentPreset, signal }) {
